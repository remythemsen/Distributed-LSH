...
import multiprobing.{TwoStepProbeGenerator, TwoStepProbeGeneratorSpec}
...
class RepetitionHandlerProbeSpec extends FlatSpec with Matchers {
  ...
  "Query " should "return 0 or more results given a valid query" in {
    ...
    val ready = a1 ? InitRepetitionProbe("data/descriptors-40000-reduced-128.data",  39290,  hashFunctions.length, "hyperplane", new TwoStepProbeGenerator(k, Array()), 100,k, 128, Euclidean, rnd.nextLong)
